...
# load pitch data
pitch <- read_csv("pitchdata.csv")

# Cleaning pitch data
pitch <- pitch %>%
  mutate(Study = substr(FileName, 0, 6),
         Diagnosis = substr(FileName, 8, 8),
         ID = substr(FileName, 10, 12),
         trial = substr(FileName, 13, 15)) %>% 
...
DEM <- read_csv("DemographicData.csv")

# then duration data
DUR <- read_csv("Articulation.txt")
DUR <- DUR %>%
  mutate(Study = substr(soundname, 0, 6),
         Diagnosis = substr(soundname, 8, 8),
         ID = substr(soundname, 10, 12), 
         trial = substr(soundname, 13, 15)
         ) %>%
  mutate(Study = ifelse(Study == "Study1", "Study 1",
                 ifelse(Study == "Study2", "Study 2",
                 ifelse(Study == "Study3", "Study 3",
...
DEM %>% 
  group_by(Diagnosis) %>%
  count()
# 59 control participants, 94 schizophrenic participants
...
DEM %>% 
  group_by(Diagnosis) %>%
  count(Gender)
# Out of 59 control participants 24 were female and 35 were male. Out of 94 schizophrenic participants 43 were female and 51 were male.

DEM %>% 
  group_by(Diagnosis) %>% 
  summarise(mean = mean(Age, na.rm = T), sd = sd(Age, na.rm = T))
# The Control group had a mean age of 23 (SD = 3.37) and the schizophrenic group had a mean age of 25 (SD = 6.10)
...
DEM %>% 
  group_by(Study, Diagnosis, Gender) %>%
  count()

DEM %>% 
  filter(Diagnosis == "Schizophrenia") %>% 
  group_by(Study) %>% 
  summarise(mean = mean(SANS, na.rm = T), sd = sd(SANS, na.rm = T))

DEM %>% 
  filter(Diagnosis == "Schizophrenia") %>% 
  group_by(Study) %>% 
  summarise(mean = mean(SAPS, na.rm = T), sd = sd(SAPS, na.rm = T))
```
